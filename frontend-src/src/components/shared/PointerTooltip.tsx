...
function release(id: string) {
  if (activeTooltip === id) claim(null);
}
...
export default function PointerTooltip({
  ...
  const place = useCallback(() => {
    ...
    const left = Math.min(
      maxLeft,
      Math.max(GUTTER, desiredLeft + box.width <= viewportWidth - GUTTER ? desiredLeft : alternateLeft),
    );
    ...
    const top = Math.min(
      Math.max(GUTTER, viewportHeight - box.height - GUTTER),
      Math.max(GUTTER, useAbove ? above : below),
    );
