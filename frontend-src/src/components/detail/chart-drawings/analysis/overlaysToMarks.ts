  ...
  return panes.map((pane) => {
    ...
    if (pane.kind === 'rsi' || pane.id === 'rsi') {
      return {
        ...
        yMax: 100,
        markLines: [30, 70],
        series: [{ key: 'rsi', name: t('RSI'), data: align('rsi'), type: 'line' }],
      };
    }
    if (pane.kind === 'macd' || pane.id === 'macd') {
      return {
        ...
        markLines: [0],
        series: [
          { key: 'macd', name: t('MACD'), data: align('macd'), type: 'line' },
          { key: 'signal', name: t('信号线'), data: align('signal'), type: 'line' },
          { key: 'histogram', name: t('柱'), data: align('histogram'), type: 'bar' },
        ],
      };
    }
    if (pane.kind === 'obv' || pane.id === 'obv') {
      return {
        ...
        label: t(pane.label),
        kind: 'obv',
        series: [{ key: 'obv', name: t('OBV'), data: align('obv'), type: 'line' }],
      };
    }
    if (pane.kind === 'clv' || pane.id === 'clv') {
      return {
        ...
        yMax: 1,
        markLines: [0],
        series: [{ key: 'clv', name: t('CLV'), data: align('clv'), type: 'line' }],
      };
    }
    if (pane.kind === 'rs' || pane.id === 'topix_rs') {
      return {
        id: pane.id,
        label: t(pane.label),
        kind: 'rs',
        markLines: [100],
        series: [{ key: 'rs', name: t(pane.label), data: align('rs'), type: 'line' }],
      };
    }
    return {
      id: pane.id,
