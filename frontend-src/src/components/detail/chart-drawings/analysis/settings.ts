...
export function loadLayerSettings(identity: string, storage?: Storage | null): LayerSettings {
  ...
  if (!store) return { ...DEFAULT_LAYER_SETTINGS, enabled: [...DEFAULT_LAYER_SETTINGS.enabled] };
  try {
    const raw = store.getItem(layersStorageKey(identity));
    if (!raw) return { ...DEFAULT_LAYER_SETTINGS, enabled: [...DEFAULT_LAYER_SETTINGS.enabled] };
