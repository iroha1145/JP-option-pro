...
Optix Japan keeps three distinct files: developer-local ``.env``,
host-specific ``machine.env`` and server-only ``secrets.env``. Values exported
by the process always win. The canonical files override ``.env`` values only
for the machine fields or secrets that belong to them — a key placed in the
wrong file is ignored rather than silently honored.
"""
