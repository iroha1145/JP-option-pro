"""Owner access control for Optix Japan.

Two deployment modes, unchanged from the proven reference design:
- ``private_network`` — the owner is whoever connects from an approved
  private CIDR; no cookies involved.
- ``password`` — one in-memory owner session at a time (PBKDF2-600k
  verified), HTTPS required for login, per-IP failure throttling.

Optix Japan is single-principal: there are no customer accounts. Visitors
in password mode get read-only access to published snapshots; every write
is owner + same-origin gated.
"""
...
OWNER_COOKIE_NAME = "optixjp_owner_session"
...
def public_snapshot_unavailable(resource: str) -> HTTPException:
    """Stable 503 used when a visitor has no saved snapshot to read."""
...
def _canonical_origin_host(value: str, *, require_ipv6: bool = False) -> str | None:
...
def _canonical_origin(value: str) -> tuple[str, str, int] | None:
    ...
    hostname = _canonical_origin_host(parsed.hostname or "", require_ipv6=bracketed)
    if (
...
def _canonical_request_origin(scheme: str, authority: str) -> tuple[str, str, int] | None:
    if (
        scheme not in _DEFAULT_ORIGIN_PORTS
        or not authority
...
class OwnerAccessRuntime:
    def __init__(
        ...
        *,
        ...
        self._networks = tuple(
            ipaddress.ip_network(value, strict=False) for value in config.allowed_private_cidrs
        )
    ...
    def validate_startup(
        ...
        *,
        ...
        if self.mode == "password":
            if not self._password_hash or not owner_password_hash_is_valid(self._password_hash):
                raise RuntimeError("ACCESS_MODE=password requires a valid APP_PASSWORD_HASH")
    ...
    def request_address(self, request: Request) -> ipaddress.IPv4Address | ipaddress.IPv6Address | None:
        raw = client_ip_from_scope(
            request.scope, enabled=TRUST_PROXY_HEADERS, networks=TRUSTED_PROXY_NETWORKS
        )
        try:
    ...
    def login(self, password: str, *, client_key: str) -> LoginResult:
        ...
        with self._lock:
            ...
            count, started_at, blocked_until = self._login_failures.get(client_key, (0, now, 0.0))
            if blocked_until > now:
                raise LoginRejected(
                    "login_cooldown", retry_after=max(1, int(blocked_until - now + 0.999))
                )
        ...
        with self._lock:
            ...
            if not valid:
                ...
                self._login_failures[client_key] = (count, started_at, blocked_until)
                raise LoginRejected(
                    "invalid_owner_password",
                    retry_after=(_LOGIN_COOLDOWN_SECONDS if blocked_until else None),
                )
    ...
    def logout(self, token: str) -> None:
        ...
        digest = hashlib.sha256(token.encode("utf-8")).digest()
        with self._lock:
            if self._session_digest is not None and hmac.compare_digest(digest, self._session_digest):
                self._session_digest = None
                self._session_expires_at = 0.0
...
def get_access_runtime() -> OwnerAccessRuntime:
    ...
    return OwnerAccessRuntime(config, password_hash=os.environ.get("APP_PASSWORD_HASH", ""))


def reset_access_runtime_for_tests() -> None:
    get_access_runtime.cache_clear()
...
def require_owner_access(request: Request) -> None:
    ...
    code = "owner_login_required" if runtime.mode == "password" else "private_network_required"
    raise HTTPException(
        status_code=(
...
async def require_public_read_or_owner_access(request: Request) -> AsyncIterator[None]:
    """Password-mode GET/HEAD reads stay public; everything else is owner-only."""

    runtime = _runtime(request)
    method = request.method.upper()
    if runtime.mode == "password" and method in {"GET", "HEAD"}:
...
def require_same_origin_request(request: Request) -> None:
    """Same-origin proof without requiring a JSON body."""
...
def require_same_origin_action(request: Request) -> None:
    if not bool(getattr(request.state, "owner_access", False)):
        require_owner_access(request)
...
__all__ = [
    ...
    "reset_access_runtime_for_tests",
