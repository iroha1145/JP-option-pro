"""Daily T1 condition helpers. Production detect_new_signal stays unchanged.

Frozen math matches option-pro PR #167 (daily_confirmation.py @ 98330ca)
with Japan algorithm version strings.
"""
...
T1_SETTINGS = {
    ...
    "clv_note": "Range-position CLV in [0, 1]. This is not radar close_location signed form.",
    "rvol_formula": "V(T) / median(V(T-20), ..., V(T-1))",
    "rvol_note": (
        "Denominator uses only the previous 20 completed sessions and share volume, "
        "never turnover_value / turnover_ratio."
    ),
    "atr": "mean(true_range, 20) on T plus the prior 20 sessions; first TR needs previous close",
    "atr_rounding": 8,
    "zero_range": "CLV and upper-shadow unavailable; T1 cannot confirm",
    "information_ready": "T regular close 15:30 Asia/Tokyo; vendor publish is a later gate",
}
...
def compute_atr(bars: Sequence[Mapping[str, Any]], period: int = 20) -> float | None:
    """Simple-mean ATR matching US ``feature_engine.compute_atr``.

    TR_t = max(H-L, |H-prev_close|, |L-prev_close|).
    ATR = mean of the last ``period`` true ranges. Needs ``period + 1`` bars
    because the first TR uses the previous close. Rounded to 8 decimal places.
    """
