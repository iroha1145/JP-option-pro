"""確定的な日足ベース（枢軸クラスタ）検出 — 米国版 base_detector の移植。

フラクタル枢軸 → 許容幅クラスタリング → 抵抗帯/支持帯 → 品質ブレンド。
参照実装との差分は (1) pandas/numpy を使わない純 Python、(2) 出来高の
代わりに売買代金（円）で contraction を測る、の2点だけ。アルゴリズムの
定数（窓グリッド・許容幅・タッチ数・品質重み）は参照版と同一。

先読み禁止: 呼び出し側は「評価対象日を除いた」バー列を渡す。当日バーが
自分の抵抗帯に参加することはない。
"""
...
DETECTOR_VERSION = "jp-base-detector-v1"
...
def _candidate(window: dict[str, list], dates: Sequence[str]) -> dict[str, Any] | None:
    ...
    if closes[-1] > resistance_high + break_buffer:
        return None  # 既に上抜けた窓は「完成したベース」ではない
...
def detect_base(series: dict[str, list]) -> dict[str, Any] | None:
    """``series`` は features.clean_series 形式（評価日を除いた列を渡すこと）。

    窓グリッドを走査し、品質が最も高い完成ベースを返す。無ければ None。
    """
...
__all__ = ["BASE_MIN_DAYS", "BREAK_BUFFER_ATR", "BREAK_BUFFER_PCT", "DETECTOR_VERSION", "WINDOW_GRID", "detect_base"]
