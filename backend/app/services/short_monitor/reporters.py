...
の対 TOPIX 超過中位）:
    ...
    海外投行 / PB 名義の増仓        ≈ −3%   （Nomura Intl −2.97 / UBS −2.92 / JPM −2.97）
    国内証券（日本語表記）・個人   ≈ 0     （SMBC日興 +0.67 / 野村證券 −0.33 / 三菱UFJMS −0.68）
    クオンツ・マーケットメイカー   強く負  （XTX −3.71 / Jane Street −3.64 / Jump −3.5）
    `Notes` の「ヘッジ」明記        区別できない（−2.02 vs −2.18）

したがって分類は **実体（名前）** で行い、`Notes` のヘッジ標注には頼らない。
分類は人手で維持する初期値であって、`app.research.informedness` の出力で
定期的に校正する前提。ここに無い名前は `unknown` —— 推測で埋めない。

`INFORMED_CLASSES` は「方向性の情報を含むとみなす」クラス。因子側は全鎖の
口径（`pressure_all`）と、このクラスだけの口径（`pressure_informed`）を
**両方** 出し、検証で別々に評価する。
"""
...
#: 外すのは、実測で予測力がほぼ無かった国内証券名義と集合名義だけ。
INFORMED_CLASSES = frozenset({
    CLASS_GLOBAL_PB, CLASS_HEDGE_FUND, CLASS_MARKET_MAKER, CLASS_UNKNOWN,
})
...
def classify(
    ...
    # （派生商品・貸株・CB のヘッジ）を映す —— 実測で予測力が無かった群。
